...
for( let index = 0 ; index < userNames.length ; index++){
    if( userNames[index].indexOf('솔') !== -1 ){console.log(userNames[index]) ;
        break;
    }
}
*/
...
for( let i = 0 ; i < seatLayout.length ; i++){
    for( let a = 0 ; j < seatLayout[i].length ; a++){
        console.log(seatLayout[i][a]);
    }
}
*/
...
let output = [] ;
for( let index = 0 ; index <= numbers.length -1 ; index++){
    if(output.indexOf(numbers[index]) == -1 ){ output.push(numbers[index])}
}
console.log(output)
*/
...
for( let i = 0 ; i < numbers.length -1 ; i++){
    ...
    for( let j = 0 ; j < numbers.length -1 ; j++){
        ...
        // console.log(j+1) // j와 인접한 인덱스 (+1)을 했으니까 마지막까지 가면 2 > ? 가 되어버림
        if( numbers[j] > numbers[j+1]){ // 만약 j값이 인접한 j+1보다 더 크면 스왑
            let temp = numbers[j]
