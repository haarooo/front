...
function findLongestWord(array){
  let longword = words[0] //첫번째 인덱스 값을 가장 긴 단어라고 가정
  for( let index = 1 ; index <= array.length -1 ; index++){
    if(longword.length < array[index].length){ 
      longword = array[index] // 가장긴 단어를 longword에 저장
    }
  }
  return longword;
}
...
function findTopStudent(){let top = students[0];
  for( let index = 1 ; index <= students.length -1 ; index ++){
    
    if(top.score < students[index].score){
      top = students[index]
    }
  }
  return top.name
...
let todoList = ['장보기', '운동하기', '일본어 공부하기'];
function renderList() {
  let html = `<ul>`;
  for (let index = 0; imdex < todoList.length; index++) {
  
    html += `<li>${todoList[index]}</li>`;
  }
  html += `</ul>`;
